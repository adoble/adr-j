...
public class CommandConfigTest {
	...
	void testDateFormat() throws Exception {
		...
		ADRProperties properties = new ADRProperties(env);
		properties.load();
		assertTrue(properties.getProperty("dateFormat").equals("ISO_LOCAL_DATE"),
				"Default date format is ISO_LOCAL_DATE");
	...
	void testIncorrectDataFormat() throws Exception {
		...
		// The dateFormat should not have been changed when an incorrect format has been
		// specified.
		properties.load();
