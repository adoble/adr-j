...
class CommandNewMetadataTest {
	...
	public void testLinkMetaData() throws Exception {
		// Create some ADRs using a template with a {{template.comment}} field so that
		// meta data is used.
		...
		assertTrue(TestUtilities.contains(
				"<!--* {{{link.comment=\"References\"}}} [ADR {{{link.id=\"1\"}}}]({{{link.file=\"0001-first-link-target-adr.md\"}}})-->",
				newADRFile));
	...
	/**
	 * Test is no links are present then the template data is not shown alone in the
	 * ADR
	 * 
	 */
	@Test
	...
	/**
	 * Test is ADR is not superseded then the template data is not shown alone in
	 * the ADR
	 * 
	 */
	@Test
